// ═══════════════════════════════════════════
//  IMPULSE — app.js  (Glassmorphism Redesign)
//  Flipbook engine + cover card parallax
// ═══════════════════════════════════════════
...
async function render3DMagazine() {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    if (i === 1 || i === totalPages) pageContainer.setAttribute('data-density', 'hard');
  ...
  pageFlipInstance = new St.PageFlip(container, {
    width: targetPageWidth, height: targetPageHeight,
    size: 'fixed', minWidth: 260, maxWidth: 600, minHeight: 350, maxHeight: 900,
    drawShadow: true, showCover: true,
    useMouseEvents: true, useTouchEvents: true,
  });
...
zoomIn.addEventListener('click',  () => { scale = Math.min(scale + 0.1, 1.5); if (pdfDoc) render3DMagazine(); });
zoomOut.addEventListener('click', () => { scale = Math.max(scale - 0.1, 0.8); if (pdfDoc) render3DMagazine(); });
...
window.addEventListener('resize', () => { if (pdfDoc) render3DMagazine(); });
...
(function initCoverParallax() {
  ...
  function animate() {
    ...
    coverCard.style.transform = `perspective(700px) rotateX(${curRX}deg) rotateY(${curRY}deg) scale3d(1.02,1.02,1.02)`;
  ...
  document.addEventListener('mouseleave', () => { targetRX = targetRY = 0; });
  ...
  coverCard.style.setProperty('--shine-x', '50%');
  coverCard.style.setProperty('--shine-y', '50%');
})();
